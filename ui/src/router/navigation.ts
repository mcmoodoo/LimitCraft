...
export const createNavigationHelpers = () => ({
  ...
  toOrderDetails: (orderHash: string) => 
    ROUTES.ORDERS.DETAIL.replace(':orderHash', orderHash),
  toOrderCancel: (orderHash: string) =>
    ROUTES.ORDERS.CANCEL.replace(':orderHash', orderHash),
